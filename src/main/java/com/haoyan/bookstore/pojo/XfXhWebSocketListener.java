...
import com.haoyan.bookstore.pojo.dto.ResponseDTO;
...
public class XfXhWebSocketListener extends WebSocketListener {
    private Logger logger = LoggerFactory.getLogger(XfXhWebSocketListener.class);
    ...
    public void onMessage(@NotNull WebSocket webSocket, @NotNull String text) {
        ...
        if (responseData.getHeader().getCode() != 0) {
            // 日志记录
            logger.error("发生错误，错误码为：" + responseData.getHeader().getCode() + "; " + "信息：" + responseData.getHeader().getMessage());
            // 设置回答
